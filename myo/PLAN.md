...
| Sidecar supervision | `DaemonChild` (kill-on-`Drop`), candidate-path resolution, probe-then-spawn; `quiet_command`; `externalBin`. | `MyOwnLLM/src-tauri/src/mesh/daemon.rs:457-526`; `.../process.rs:20-49`; `.../tauri.conf.json:38-40` |
...
**Tier-a category → tools (v1):** **Web** = `web_search`, `builtin_browser` (+ research; `allow_web_search`); **Files** = `read_file`, `write_file`, `create/edit/update/suggest_document`; **Code** = `bash`, `python` (`allow_bash`); **Reach-out** = `send_email`, `reply_to_email`, `list/read_email`, `manage_calendar`, `resolve/manage_contact`. Default posture: **Web on, others off**. Management/infra/vault tools (`manage_*`, `vault_*`, `api_call`, model-serving) map to no v1 category → disabled by default; admin/public role is the backstop.
...
**2. Extract `crates/myo-asr`** (reuse). Move (history-preserving) from `MyOwnLLM/src-tauri/src/`: `asr/*`, `diarize/*`, `transcribe.rs`, `frame_sink.rs`, `resolver.rs`, `hardware.rs`, `models.rs`, `ort_setup.rs`, `ort_install.rs`. Delete `impl FrameSink for WebviewWindow` (`frame_sink.rs:31-40`); change `transcribe.rs` start/upload signatures to `sink: Arc<dyn FrameSink>` (bodies already call `sink.emit_frame`). Fast-follow: point MyOwnLLM at the crate to de-dup.

**3. Engine supervision** (`supervisor/`, porting `mesh/daemon.rs` + `process.rs`): `odysseus.rs` probes `GET /api/health` then spawns `uvicorn app:app --host 127.0.0.1 --port 7000` (cwd=bundled dir) with env `ODYSSEUS_INTERNAL_TOKEN=<random>`, `AUTH_ENABLED=true`, `ODYSSEUS_INPROCESS_POLLERS=0`, `ODYSSEUS_INPROCESS_TASKS=0`; kill-on-`Drop`. `myownllm.rs` spawns bundled `myownllm serve --port 1473`, polls `/healthz`. Progress on `myo://engine/*`.

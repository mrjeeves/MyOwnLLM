...
import { APP_VERSION } from "../../mesh-capabilities";
  ...
  // Pending-approval list now includes peers we've approved that are
  // still waiting for the peer's reciprocal approve. Substrate's
  // bilateral-approval model (cf. MyOwnMesh's ApprovalsSection)
  // surfaces three distinct sub-states the UI needs to render
  // differently — `approvalState(p)` below collapses the booleans
  // into one of "fresh" / "confirm-needed" / "waiting-peer".
  let pendingRequests = $derived(
    meshClient.peers.filter(
      (p) =>
        p.status === "pending_approval" ||
        (p.status === "pending_remote" && p.local_approved),
    ),
  );

  /** Map a peer's `(local_approved, remote_approved)` pair into one
   *  of three rendering states. Mirrors `approvalState` in
   *  `MyOwnMesh/gui/src/ui/settings/ApprovalsSection.svelte` —
   *  keeping the names aligned makes it easier to compare flows
   *  across the two products.
   *
   *  - "fresh": neither side approved yet. Show the full bilateral
   *    confirmation grid + primary Approve / Deny.
   *  - "confirm-needed": peer approved first, we haven't. Stronger
   *    callout ("peer authorised you — confirm to finish") plus the
   *    same tiles + Approve / Deny. Was MyOwnLLM's
   *    `!approver_role` branch.
   *  - "waiting-peer": we approved, peer hasn't. Show "waiting for
   *    peer …" with a Revoke escape hatch that drops the roster
   *    entry + tears down the connection. */
  function approvalState(p: (typeof meshClient.peers)[number]):
    | "fresh"
    | "confirm-needed"
    | "waiting-peer" {
    if (p.local_approved && !p.remote_approved) return "waiting-peer";
    if (!p.local_approved && p.remote_approved) return "confirm-needed";
    return "fresh";
  }
                ...
                {#if state === "waiting-peer"}
                  <div class="confirm-row">
                    <div class="confirm-help">
                      You've approved this peer. Waiting for them to
                      send their approve so the handshake completes.
                      If they never confirm, revoke to drop the
                      pending request — they'll need to re-knock.
                    </div>
                  </div>
                {:else}
                  <div class="confirm-row">

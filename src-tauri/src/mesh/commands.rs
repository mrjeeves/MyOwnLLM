...
//!
//! Every command here is a thin Tauri-bridge wrapper. The actual
//! crypto and serialization logic lives in `myownmesh-core`, so the
//! same byte-for-byte rules apply across every product in the family
//! (MyOwnLLM, MyOwnMesh, …). The substrate is the single source of
//! truth for: ed25519 signing / verification, base32 device id
//! encoding, verification-code alphabet + length, network-id
//! normalization, and the roster JSON schema.

use serde::Serialize;

use super::{identity, roster, signing};
use myownmesh_core::verification;
...
// ---- verification codes (out-of-band confirmation) --------------------
//
// Both ends of a fresh peer handshake exchange 6-char codes from
// [a-z0-9] alongside their device ids. The user reads the codes
// aloud over voice/video to confirm "you're approving the request
// that you actually sent" before the bilateral approve flips the
// peer to Active. Generation + validation live in the substrate
// (`myownmesh_core::verification`) so every consumer agrees on the
// alphabet, length, and well-formedness rules — a code minted by
// one product is recognised by the other without translation.

/// Generate a fresh 6-char verification code from OS randomness.
/// The same alphabet and length the substrate emits, so codes
/// generated here are indistinguishable from codes generated by
/// any other myownmesh consumer.
#[tauri::command]
pub fn mesh_verification_code_generate() -> String {
    verification::generate_code()
}

/// Sanity-check a code received over the wire. Used by the
/// approval UI to surface "[malformed]" instead of a blank tile
/// when a peer sends a degenerate code (empty, oversize, wrong
/// alphabet). The ed25519 mutual signature is what actually
/// authenticates the peer; this is a UX guard rail.
#[tauri::command]
pub fn mesh_verification_code_is_well_formed(code: String) -> bool {
    verification::is_well_formed(&code)
}

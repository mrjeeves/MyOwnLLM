...
The **Cloud Mesh** is the distributed-intelligence half. Devices on the same Network ID find each other over public Nostr relays (or your own self-hosted one), authenticate via mutual ed25519, and share work peer-to-peer over WebRTC: chat routes through any peer's LLM, Talking Points run on whichever device is holding the chat model warm, and the transcribe protocol surface is wired (the Rust audio pipeline lands in a follow-up). Routing is **per-surface and persistent** — each pane has its own host pin, stored by device pubkey so reconnects and reloads don't drop it. A pinned peer dropping pauses the work tied to it instead of silently downgrading to the local LLM the user didn't pick.
...
| `mesh-protocol.ts` | Wire-protocol types and pure helpers: `MeshMessage` union (hello, auth_response, approve, deny, ping, pong, capabilities_update, shelve/unshelve, catalog_announce, move_*, move_prepare/commit/abort, infer_request/chunk/done/error/cancel, file_offer/accept/decline/chunk/complete/abort, transcribe_request/audio_chunk/segment/done/error/cancel), `Capabilities` shape (LLMs / ASR / hardware / inputs / outputs / accepting / `app_version` / `features`), base32 encode, nonce + verification code + mesh-id generation, `selectRingNeighbors` (pure ring-topology selector), `pubkeyPart` / `pubkeySuffix`, `authPayload`, `deriveNetworkHandle`, the `FEATURES` registry (stable string ids for optional message kinds, including `REMOTE_TRANSCRIBE`), `ADVERTISED_FEATURES` (what this build supports), `peerSupportsFeature(cap, id)` (the gate every optional send checks), `summarizePeerCompat(cap)` (helper for the UI's compatibility chip), `FILE_CHUNK_BYTES` / `FILE_MAX_BYTES`, `TRANSCRIBE_SAMPLE_RATE` / `TRANSCRIBE_AUDIO_CHUNK_BYTES`. No runtime state; safe to import from anywhere. |
| `mesh-capabilities.ts` | Snapshot the local capability surface (`detect_hardware` + `ollama_list_models` + `asr_models_list` + `audio_input_devices`) into the wire shape — stamps in the build-time `APP_VERSION` and `ADVERTISED_FEATURES` on every snapshot so peers always see this device's current matrix. Provides `summarizeCapabilities`, `capabilityBadges`, `canServeInference`, `canServeTranscribe`, plus `formatPeerCompat(cap)` / `describePeerMissingFeatures(cap)` for the Connections card and sidebar compat pills. |
| `mesh-client.svelte.ts` | The mesh runtime. Owns the Trystero room (strategy chosen at build time via `VITE_TRYSTERO_STRATEGY`, lazy-loaded via `loadJoinRoom`), the per-peer `ConnectionState` map, the bidirectional auth handshake, ring topology evaluation, capability snapshot + broadcast, catalog gossip (debounced on mutation), the 2-phase Move RPC (push) and the Pull RPC, remote inference dispatch (outbound + inbound), file transfer (offer/chunk/complete with SHA verification + save-dialog handoff), the remote-transcribe RPC (sender path + receiver skeleton — Rust audio pipeline lands in a follow-up), a `catalog_cache` and `capabilities_cache` keyed by pubkey so offline rostered peers still render their last-known LLM/ASR in selectors and the sidebar, the resource-map tracking (in-flight inferences + moves + files, in both directions), and the heartbeat / wake / re-handshake / forced-rediscovery resilience layer. Every optional send is gated on `peerSupportsFeature(conn.capabilities, FEATURES.X)` so peers that don't implement a kind don't get spammed with frames they'd drop. Exposes reactive `peers`, `my_capabilities`, `my_catalog`, `accepting`, `diag_quiet`, `resources`, `files`, `inbound_offers` for the GUI to subscribe to. Public methods include `reconcile`, `start`, `stop`, `approveRequest`, `denyRequest`, `removePeer`, `reconnectPeer`, `forceRediscovery`, `moveConversation` (push), `pullConversation`, `sendInferRequest`, `sendTranscribeRequest`, `sendFile`, `acceptInboundFile`, `declineInboundFile`, `noteCatalogChanged`, `noteCapabilitiesChanged`, `setAccepting`, `setDiagQuiet`. |
| `ui/TopBar.svelte`, `TextBar.svelte`, `TranscribeBar.svelte`, `ModelSelector.svelte` | Workspace chrome. TopBar (hamburger + Text/Transcribe mode buttons with slot status indicators + Settings icon) replaces the old StatusBar. TextBar (model selector + brain toggle + context ring) replaces the bottom row of the old ModeBar. TranscribeBar (model selector only) renders twice in transcribe mode — under each pane. ModelSelector is the reusable picker shared across all three: renders as a styled pill when local-only and as a `<select>` once any peer can serve the kind, taking and emitting a stable `device_pubkey` so pins survive Trystero session changes. |
...
| `ui/settings/CloudMeshSection.svelte` | Sub-tab strip for the **Networks** settings tab (renamed from "Cloud Mesh"; internal id stays `cloud-mesh`). Renders five panes: **Status** (identity, status pill, saved networks, pending requests), **Connections** (ring + indirect + resource map), **Activity** (diagnostic log), **Settings** (per-network signaling / STUN / TURN), **HTTP** (the axum-served browser UI, previously labeled "LAN"). Takes an `initialSubTab` prop so deep-links from `settingsRoute` land on the right pane. |
| `ui/settings/CloudMeshStatus.svelte` | Home view: single-line identity card (label · suffix · device_id), status pill with inline accepting dropdown, saved-networks list with Switch (inactive rows) and Forget (every row, guarded by a confirmation modal), + Add network button, pending Network requests when present. Includes inline hint when 3+ pending requests pile up (steer toward a more unique Network ID). |
...
| `ui/settings/CloudMeshActivity.svelte` | Ring-buffered diagnostic log (80-entry cap, info/warn/error levels) with a quiet-logs checkbox that suppresses `info` events. Moved off Status so steady-state chatter doesn't crowd the controllable surfaces. |

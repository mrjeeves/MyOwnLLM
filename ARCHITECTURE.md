...
| `ui/settings/CloudMeshStatus.svelte` | Home view: single-line identity card (label · suffix · device_id), status pill with inline accepting dropdown, saved-networks list with per-network lock toggle / switch / forget / + Add, pending Network requests when present. Includes inline hint when 3+ pending requests pile up (steer toward a more unique Network ID). |
...
| `ui/settings/AddNetworkModal.svelte` | Single-input modal (Network ID + Generate) for saving a new mesh network. Three save modes: Save, Save & activate, Save & start (= activate + lock). ⌘/Ctrl + Enter shortcut on Save & start. Mounted from the Status tab's "+ Add network" button. |
